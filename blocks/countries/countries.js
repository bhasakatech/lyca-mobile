export default function decorate(block) {
  ...
  rows.forEach((row, index) => {
    if (index === 0) {
      row.replaceWith(createButton('cntry-btn-next', '→'));
    } else if (index === rows.length - 1) {
      row.replaceWith(createButton('cntry-btn-prev', '←'));
    } else {
      row.classList.add('country-slide');
    }
  });

  // Select only slides inside the current block
  const slides = block.querySelectorAll('.country-slide');

  let currentSlide = 0;
  const totalSlides = slides.length - 1;

  // Select buttons from within block
  const nextBtn = block.querySelector('.cntry-btn-next');
  const prevBtn = block.querySelector('.cntry-btn-prev');
